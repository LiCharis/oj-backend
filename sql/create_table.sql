...
(
    ...
    use rPassword varchar(512)                           not null comment '密码',
...
(
    id         bigint auto_increment comment 'id' primary key,
    content    text                               null comment '内容',
    tags       varchar(1024)                      null comment '回复评论ids列表（json 数组）',
    thumbNum   int      default 0                 not null comment '点赞数',
    userId     bigint                             not null comment '创建用户 id', FOREIGN KEY (userId) REFERENCES user(id),
